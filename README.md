...
- **Без подписки** после первого полноценного ответа дальше — только экран с кнопками оплаты (и без `PAYMENT_PROVIDER_TOKEN` это **тестовый режим**: кнопки есть, счёт не откроется, пока не зададите токен). Тарифы **100 ₽ / месяц** или **600 ₽ / год** (копейки в `.env`).
...
2. В `.env`: `PAYMENT_PROVIDER_TOKEN=...`, при необходимости `SUBSCRIPTION_PRICE_KOPECKS=10000` (100 ₽/мес.), `SUBSCRIPTION_YEAR_PRICE_KOPECKS=60000` (600 ₽/год).
